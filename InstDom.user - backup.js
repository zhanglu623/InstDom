...
var count_check_event_handler_change = 0;
...
function check_event_handler_change() {
	...
	count_check_event_handler_change++;
	// console.log("-----" + count_check_event_handler_change
	// + ": check_event_handler_change();------");
	...
	function step1_classify_ehs(element, curr_eh, eh_type) {
		...
		if (element.ehs[eh_type]) {
			...
			if (old_eh == curr_eh.toString()) {
				...
				console.log("-->Different old event handler: \n::" + old_eh
						+ "\n--- VS ---\n::"
						+ curr_eh.toString());
